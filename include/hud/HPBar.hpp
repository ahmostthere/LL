...
struct HPBar : public sf::Drawable {
    HPBar(sf::Vector2f _size = sf::Vector2f(100, 10), float _totalHP = 100,
...
HPBar::HPBar(sf::Vector2f _size, float _totalHP,
             float _currentHP)
: totalHP(_totalHP), currentHP(_currentHP), containerSize(_size) {
    init();
}
...
void HPBar::update() {
    ...
    currentHPBar.setSize(
        sf::Vector2f(containerSize.x * percentHP, containerSize.y));
}

void HPBar::setPosition(const sf::Vector2f &_position) {
    position = _position;
    containerBar.setPosition(_position);
    currentHPBar.setPosition(_position);
}

void HPBar::setHPColor(const sf::Color &_color) {
    color = _color;

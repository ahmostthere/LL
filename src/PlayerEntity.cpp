...
void PlayerEntity::showIndicator() {
    m_showIndicator = true;
}

void PlayerEntity::hideIndicator() {
    m_showIndicator = false;
}
...
void PlayerEntity::setPosition(sf::Vector2f position) {
...
sf::Vector2f PlayerEntity::getSize() const {
...
void PlayerEntity::update(sf::Time time) {
    ...
    m_face.setOrigin(5 - std::cos(Compass::toRadian(facing)) * 25,
                     72 - std::sin(Compass::toRadian(facing)) * 18);
}

void PlayerEntity::printFace() {
    Compass::Direction facing = DPadMovable::getDirection();
    switch (facing) {

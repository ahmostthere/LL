...
sf::RenderTexture r_tex;
std::vector<sf::Vector2f> dotpos;
std::vector<sf::Color> dotcol;
...
HexagonShape hex(100, 40);
HexagonShape hex2(100, 0);
...
void Game::load() {
    ...
    m_window.create(sf::VideoMode(gameSettings.width, gameSettings.height),
                    gameSettings.title);
	...
	// Shadow testing
	r_tex.create(m_window.getSize().x, m_window.getSize().y);
    ...
    camBound =
        CameraBound(sf::Vector2f(gameSettings.width, gameSettings.height));
    ...
    player.setPosition(
        sf::Vector2f(gameSettings.width / 2, gameSettings.height / 2));
    environment.addEntity(&player);

    // HP Bar
    hpbar =
        HPBar(sf::Vector2f(gameSettings.width / 5, gameSettings.height / 20));
	...
	hex.setPosition(sf::Vector2f(400, 400));
	hex.setFillColor(sf::Color::Magenta);
	hud.addComponent(&hex);
	hex2.setPosition(sf::Vector2f(600, 400));
	hex2.setFillColor(sf::Color::Cyan);
	hud.addComponent(&hex2);
}
        ...
        switch (currentEvent.type) {
            ...
            case sf::Event::KeyPressed: {
                switch (currentEvent.key.code) {
                    ...
                    case (sf::Keyboard::Space): {
                        camBound.setPosition(
                            mainView.getCenter().x - camBound.getSize().x / 2,
                            mainView.getCenter().y - camBound.getSize().y / 2);
                        break;
					}

                    case (sf::Keyboard::B): {
						dotpos.push_back(m_window.mapPixelToCoords(sf::Mouse::getPosition(m_window)));
						dotcol.push_back(sf::Color(0, 0, 128, 255));
                        break;
                    }
                    case (sf::Keyboard::G): {
						dotpos.push_back(m_window.mapPixelToCoords(sf::Mouse::getPosition(m_window)));
						dotcol.push_back(sf::Color(0, 128, 0, 255));
                        break;
                    }
                    case (sf::Keyboard::R): {
						dotpos.push_back(m_window.mapPixelToCoords(sf::Mouse::getPosition(m_window)));
						dotcol.push_back(sf::Color(128, 0, 0, 255));
                        break;
                    }
                    ...
                    case (sf::Keyboard::V): {
                        ...
                        std::ofstream file(
                            "assets/saves/" + std::to_string(now) + "_out.json",
            ...
            case sf::Event::MouseButtonPressed: {
                if (sf::Mouse::isButtonPressed(sf::Mouse::Right)) {
                    sf::Vector2f dest_pos = sf::Vector2f(
                        sf::Mouse::getPosition(m_window).x +
                            mainView.getCenter().x -
                            m_window.getDefaultView().getCenter().x,
                        sf::Mouse::getPosition(m_window).y +
                            mainView.getCenter().y -
                            m_window.getDefaultView().getCenter().y);
...
void Game::render() {
	...
	sf::CircleShape tex_circ;
	tex_circ.setRadius(25);
	tex_circ.setOrigin(25, 25);

	// r_tex.clear(sf::Color::Black);
	r_tex.clear(sf::Color(255, 255, 255, 128));
	for (int i = 0; i < dotpos.size(); i++) {
		tex_circ.setPosition(dotpos[i]);
		tex_circ.setFillColor(dotcol[i]);
		r_tex.draw(tex_circ, sf::BlendMode(
	...
	r_tex.display();
	m_window.draw(sf::Sprite(r_tex.getTexture()), sf::BlendMultiply);

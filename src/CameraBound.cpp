...
CameraBound::CameraBound(sf::Vector2f wSz, float spd) : windowSize(wSz), size(sf::Vector2f(wSz.x / 8, wSz.y / 8)), speed(spd)
{
...
{
    m_cameraView = view;
    r.setPosition(m_cameraView->getCenter().x - size.x / 2, m_cameraView->getCenter().y - size.y / 2);
}

void CameraBound::boundVertical(sf::Time time)
{
    if (m_cameraView->getCenter().y + getSize().y / 2 <= m_subject->getPosition().y + m_subject->getSize().y / 2 || m_cameraView->getCenter().y - getSize().y / 2 >= m_subject->getPosition().y - m_subject->getSize().y / 2)
    {
...
{
    if (m_cameraView->getCenter().x + getSize().x / 2 <= m_subject->getPosition().x + m_subject->getSize().x / 2 || m_cameraView->getCenter().x - getSize().x / 2 >= m_subject->getPosition().x - m_subject->getSize().x / 2)
    {
